...
class BookingDAO(BaseDAO):
    ...
    async def add(cls, user_id: int, room_id: int, date_from: date, date_to: date):
        ...
        # )
        # select
        # rooms.quantity - count(booked_rooms.room_id)
        # from rooms leftjoin
        # booked_rooms
        # on
        ...
        # rooms.id = 1
        # group by
        # rooms.quantity, booked_rooms.room_id
        try:
            ...
            if isinstance(e , SQLAlchemyError):
                msg = 'Database exception : Сannot add Booking'
    ...
    async def delete(cls , user_id: int):
        try:
            async with async_session_maker() as session:
                deletes = (delete(Bookings).where(Bookings.user_id == user_id))
                new_deletes = await session.execute(deletes)
                await session.commit()
                return new_deletes.scalar()
        except(SQLAlchemyError , Exception) as e :
